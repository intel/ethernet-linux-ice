			 ...
			 struct ice_aqc_list_caps_elem *cap)
{
	...
	func_p->ts_func_info.master_tmr_owned =
		((number & ICE_TS_MASTER_TMR_OWND_M) != 0);
	...
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: ieee_1588 = %d\n",
		  func_p->common_cap.ieee_1588);
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: master_tmr_owned = %d\n",
		  func_p->ts_func_info.master_tmr_owned);
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: tmr_ena = %d\n",
		  func_p->ts_func_info.tmr_ena);
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: tmr_index_owned = %d\n",
		  func_p->ts_func_info.tmr_index_owned);
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: clk_freq = %d\n",
		  func_p->ts_func_info.clk_freq);
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: clk_src = %d\n",
		  func_p->ts_func_info.clk_src);
	ice_debug(hw, ICE_DBG_INIT,
		  "func caps: tmr_index_assoc = %d\n",
		  func_p->ts_func_info.tmr_index_assoc);
}
	       ...
	       enum ice_fc_mode req_mode)
{
	...
	pcaps = devm_kzalloc(ice_hw_to_dev(pi->hw), sizeof(*pcaps),
			     GFP_KERNEL);
	if (!pcaps)
		return ICE_ERR_NO_MEMORY;
...
out:
	devm_kfree(ice_hw_to_dev(pi->hw), pcaps);
	return status;
}
...
void ice_dump_ptp_func_caps(struct ice_hw *hw)
{
	...
	dev_info(ice_hw_to_dev(hw), "PTP func cap: master_tmr_owned = %d\n",
		 ptpfunc->master_tmr_owned);
