...
/* This is taken from upstream <drivers/gnss/core.c> commit 23680f0 */
...
#else /* !HAVE_STREAM_OPEN */
	nonseekable_open(inode, file);
#endif /* HAVE_STREAM_OPEN */
...
static ssize_t gnss_read(struct file *file, char __user *buf, size_t count,
			 loff_t *pos)
{
	struct gnss_device *gdev = file->private_data;
	unsigned int copied;
...
#ifdef HAVE_CDEV_DEVICE
	ret = cdev_device_add(&gdev->cdev, &gdev->dev);
	if (ret) {
		dev_err(&gdev->dev, "failed to add device: %d\n", ret);
		return ret;
	}
#else /* !HAVE_CDEV_DEVICE */
	if (gdev->dev.devt) {
		WARN_ON(!gdev->dev.kobj.state_initialized);
		gdev->cdev.kobj.parent = &gdev->dev.kobj;

		ret = cdev_add(&gdev->cdev, gdev->dev.devt, 1);
		if (ret)
			goto err_cdev_del;
	}
